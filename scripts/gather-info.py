...
def main():
    parser = argparse.ArgumentParser(description="Gather information from a raspberry pi system")
    group = parser.add_mutually_exclusive_group(required=True)
    group.add_argument("-all", help="show all information", action='store_true')
    group.add_argument("-clock", help="show clock information ", action='store_true')
    group.add_argument("-disk", help="show disk information", action='store_true')
    group.add_argument("-display", help="show display information", action='store_true')
    group.add_argument("-essentials", help="show temperature/memory/display/network information", action='store_true')
    group.add_argument("-memory", help="show memory information", action='store_true')
    group.add_argument("-network", help="show network information", action='store_true')
    group.add_argument("-system", help="show system information", action='store_true')
    group.add_argument("-thermal", help="show temperature/thermal information [units: celsius]", action='store_true')
    group.add_argument("-voltages", help="show voltages information [units: volts]", action='store_true')
    args = parser.parse_args()
    raspi = RaspberryPi()
    if args.all:
        print(json.dumps(raspi.gather_all_info()))
    if args.clock:
        print(json.dumps(raspi.get_clock_frequencies()))
    if args.disk:
        print(json.dumps(raspi.get_disk_stats()))
    if args.display:
        print(json.dumps(raspi.get_display_info()))
    if args.essentials:
        print(json.dumps(raspi.get_essentials()))
    if args.memory:
        print(json.dumps(raspi.get_memory_info()))
    if args.network:
        print(json.dumps(raspi.get_network_info()))
    if args.system:
        print(json.dumps(raspi.get_system_info()))
    if args.thermal:
        print(json.dumps(raspi.get_temperature()))
    if args.voltages:
        print(json.dumps(raspi.get_voltages()))
